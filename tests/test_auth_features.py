...
class TestPasswordReset:
    def test_forgot_password_known_email(self, client, regular_user, db):
        res = client.post("/api/v1/auth/forgot-password", json={"email": "test@test.com"})
    ...
    def test_reset_password_with_valid_token(self, client, regular_user, db):
        client.post("/api/v1/auth/forgot-password", json={"email": "test@test.com"})
        db.refresh(regular_user)
        res = client.post(
...
class TestEmailVerification:
    def test_send_and_verify_email(self, client, user_token, regular_user, db):
        res = client.post("/api/v1/auth/send-verify-email", headers=auth_header(user_token))
        assert res.status_code == 200
        db.refresh(regular_user)
        assert regular_user.verify_token

        res = client.post("/api/v1/auth/verify-email", json={"token": regular_user.verify_token})
        assert res.status_code == 200
        db.refresh(regular_user)
...
class TestOtp:
    def test_full_otp_lifecycle(self, client, user_token, regular_user, db):
        res = client.post("/api/v1/auth/otp/generate", headers=auth_header(user_token))
        assert res.status_code == 200
        ...
        res = client.post(
            "/api/v1/auth/otp/verify", json={"otp": code}, headers=auth_header(user_token)
        )
        assert res.status_code == 200
        db.refresh(regular_user)
        ...
        res = client.post("/api/v1/auth/otp/disable", headers=auth_header(user_token))
        assert res.status_code == 200
        db.refresh(regular_user)
    ...
    def test_verify_with_wrong_code(self, client, user_token):
        client.post("/api/v1/auth/otp/generate", headers=auth_header(user_token))
        res = client.post(
            "/api/v1/auth/otp/verify", json={"otp": "000000"}, headers=auth_header(user_token)
        )
        assert res.status_code == 401

    def test_validate_without_setup(self, client, user_token):
        res = client.post(
            "/api/v1/auth/otp/validate", json={"otp": "123456"}, headers=auth_header(user_token)
        )
        assert res.status_code == 400
