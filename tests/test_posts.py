...
class TestPosts:
    def test_create_post(self, client, user_token):
        res = client.post("/api/v1/posts", json={
            "title": "Test Post",
    ...
    def test_list_posts(self, client, user_token):
        client.post("/api/v1/posts",
                    json={"title": "Post 1"}, headers=auth_header(user_token))
        client.post("/api/v1/posts",
                    json={"title": "Post 2"}, headers=auth_header(user_token))
    ...
    def test_list_posts_includes_user_and_comments(self, client, user_token):
        client.post("/api/v1/posts",
                    json={"title": "Post"}, headers=auth_header(user_token))
        res = client.get("/api/v1/posts")
        post = res.json()["data"][0]
        assert "user" in post
        assert "comments" in post

    def test_current_user_posts(self, client, user_token):
        client.post("/api/v1/posts",
                    json={"title": "My Post"}, headers=auth_header(user_token))
        res = client.get("/api/v1/posts/current-user",
                         headers=auth_header(user_token))
        assert res.status_code == 200
    ...
    def test_update_post_forbidden(self, client, user_token, second_user_token):
        ...
        post_id = create_res.json()["data"]["id"]
        res = client.patch(
            f"/api/v1/posts/{post_id}", json={"title": "Hacked"}, headers=auth_header(second_user_token))
    ...
    def test_delete_post(self, client, user_token):
        ...
        post_id = create_res.json()["data"]["id"]
        res = client.delete(
            f"/api/v1/posts/{post_id}", headers=auth_header(user_token))
        assert res.status_code == 200
    ...
    def test_delete_post_forbidden(self, client, user_token, second_user_token):
        ...
        post_id = create_res.json()["data"]["id"]
        res = client.delete(
            f"/api/v1/posts/{post_id}", headers=auth_header(second_user_token))
        assert res.status_code == 403
    ...
    def test_admin_can_update_any_post(self, client, user_token, admin_token):
        ...
        post_id = create_res.json()["data"]["id"]
        res = client.patch(
            f"/api/v1/posts/{post_id}", json={"title": "Admin Edit"}, headers=auth_header(admin_token))
        assert res.status_code == 200
