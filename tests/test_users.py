...
class TestCurrentUser:
    ...
    def test_delete_current_user(self, client, user_token):
        res = client.delete("/api/v1/users/current", headers=auth_header(user_token))
        assert res.status_code == 200
...
class TestUserPosts:
    def test_get_user_posts(self, client, user_token, regular_user):
        res = client.get(f"/api/v1/users/{regular_user.id}/posts", headers=auth_header(user_token))
        assert res.status_code == 200

    def test_get_user_role(self, client, user_token, regular_user):
        res = client.get(f"/api/v1/users/{regular_user.id}/roles", headers=auth_header(user_token))
        assert res.status_code == 200
...
class TestAdminUserOps:
    ...
    def test_get_user_by_id(self, client, admin_token, regular_user):
        res = client.get(f"/api/v1/users/{regular_user.id}", headers=auth_header(admin_token))
        assert res.status_code == 200
        assert res.json()["data"]["user"]["username"] == "testuser"

    def test_get_user_not_found(self, client, admin_token):
        res = client.get("/api/v1/users/nonexistent-id", headers=auth_header(admin_token))
        assert res.status_code == 404
    ...
    def test_admin_delete_user(self, client, admin_token, regular_user):
        res = client.delete(f"/api/v1/users/{regular_user.id}", headers=auth_header(admin_token))
        assert res.status_code == 200
